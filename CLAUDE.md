...
- [mkdocs.yml](mkdocs.yml) — site config: theme + two-palette scheme (light first, so it is the default; no `media:` on either entry, so nothing follows the OS "auto" preference — the site is deterministically light until toggled; the palette entries keep their `toggle:` blocks only so Material renders the `__palette` radio group that the `t` key advances, the button itself being hidden by extra.css — `t` reads the scheme applied to `<body>` to find the current palette, since Material leaves the radios unchecked on a fresh visit), `theme.custom_dir: overrides` (the 404 template and the pre-paint `kiln-js` script), features (`navigation.instant`, `navigation.prune`, `navigation.footer` for key-nav.js, etc.), markdown extensions, `extra_javascript` load order, and `validation` settings. The search plugin stays although Material's search UI is hidden — quick-jump.js reads the `search_index.json` it emits. Link problems are escalated to warnings so `--strict` fails on them — including cross-page anchor fragments (`validation.links.anchors`); the homepage is intentionally not in `nav`, so `nav.omitted_files` stays at `info`. `theme.font: false` disables Material's Google-Fonts loading — JetBrains Mono comes from the `@import` in extra.css instead. Two `extra_javascript` order constraints: page-utils.js stays first (the shared `KilnUtils` global), and quick-jump.js precedes ascii-logo.js (their page-change hooks run in load order, so the homepage jump bar is in the DOM before the logo sizes itself against the article's remaining free height). The `nav:` section must be updated manually when adding pages or sections.
...
- [docs/stylesheets/extra.css](docs/stylesheets/extra.css) — all custom styling: JetBrains Mono as the global font, color variables in `:root` with `[data-md-color-scheme="slate"]` dark-mode overrides, ASCII logo sizing variables, card grid, register cards, sidebar styling (sticky nav titles with a pure-CSS `::after` gradient fade below them — no JS involved), the hex progress chip, and the palette's `:` command output. The Material footer is hidden entirely (`.md-footer { display: none }`) — its prev/next links still feed key-nav.js. Notes:
    ...
    - The sidebar scroll container (and the palette's results dropdown) carry `overscroll-behavior: contain`, so a wheel gesture that runs them to their end never chains into scrolling the main page.
...
- [docs/javascripts/page-chrome.js](docs/javascripts/page-chrome.js) — the hex reading-progress readout (`.scroll-progress`, fixed bottom-right: scroll offset over full scroll range as lowercase hex byte offsets, `0x1388/0x8d88`, offset padded to the range's width so the chip never resizes; empty on unscrollable pages). The chip shows only around scroll activity — `.scroll-progress--visible` is toggled on scroll and dropped ~1s after scrolling stops. The scroll range is cached (measured on init/resize/page change, re-synced after each scroll burst), so the per-scroll-event path never reads `scrollHeight` — that read forces a layout flush per frame during wheel scrolling. Init is idempotent and re-runs via `KilnUtils.onPageChange` because `navigation.instant` removes injected DOM on navigation.
- [docs/javascripts/key-nav.js](docs/javascripts/key-nav.js) — keyboard navigation, replacing the former scroll-to-top/bottom buttons, with the scroll motions case-accurate vim: `gg` (two-tap chord) / `G` jump to top/bottom, lowercase `j`/`k` step down/up (uppercase forms deliberately dead — vim means other things by them), `[`/`]` jump to the previous/next `h2` section (anchor line just below the sticky header), `<`/`>` follow the previous/next page in nav order, `0` goes home (clicks the header `md-logo` link), `1`–`6` open the top-level categories in sidebar nav order (clicks the nth top-level `md-nav__item` link), `t` cycles the color palette (advances Material's `__palette` radio group — light and dark only; the header toggle button is hidden), `s` reveals/re-hides both sidebars, which are **hidden by default site-wide** and **disabled entirely on the homepage** (`kiln-sidebars-shown` on `body` — desktop-only in extra.css because the sidebars are off-canvas drawers below Material's 76.25em breakpoint; the class survives `navigation.instant`, so the choice holds across pages until reload, and never applies on the homepage), there is no help binding — help is the palette's `:h`/`:help` usage output (see quick-jump.js, which owns the binding table those commands print; keep it in sync when bindings change). All letter bindings are strictly lowercase (uppercase forms are inert, as in vim). Page switching clicks Material's footer prev/next links (`md-footer__link--prev/next`), which exist in the DOM only because `navigation.footer` is enabled in mkdocs.yml — the footer itself stays `display: none`; the click path keeps `navigation.instant` in charge. Keys are inert while typing and in Ctrl/Alt/Meta chords; Material's own `P`/`N` (prev/next) bindings stay, while its search keys `s`/`S`/`F`/`/` are all neutralized by a capture-phase listener, so nothing summons the hidden search form — lowercase `s` performs the sidebar toggle from that listener (a bubble-phase binding would lose the race to Material's earlier-registered handler), the rest are dead keys, and the palette belongs to `` ` `` alone (all still type in inputs); scrolling is smooth unless `prefers-reduced-motion`. Attach-once (no per-page init); the help panel lives on `document.body`, surviving instant navigation.
- [docs/javascripts/quick-jump.js](docs/javascripts/quick-jump.js) — jump-to-page palette, the site's navigation and its search (Material's search UI is hidden; the search plugin only feeds this). `` ` `` or Ctrl/Cmd+K summons the prompt — the homepage's inline bar when mounted, an overlay elsewhere: a typed query searches everything — pages fuzzy-matched by title and path rank first (substring hits outrank subsequence scatter; whitespace-separated terms must all match, so `csapp 3` works), then section-content hits fill the remaining rows as `page › section` rows with a hit excerpt (literal substrings only in body text, deduped and capped at two rows per page), so a term living only in body text (`memcmp`, `0x5A`) surfaces without any prefix — ArrowDown/ArrowUp select, Enter clicks the result's real link (keeping `navigation.instant` in charge), Escape or a backdrop click closes; the prompt rests as a bar showing a `█` terminal cursor (steady while unfocused, blinking while focused, the Linux console convention) plus a faint `type :h for help` placeholder (an empty query still shows no results; the block cursor hides and the native caret takes over at the first typed character, driven by `:placeholder-shown`), and the current page is never offered. A leading `:` is vim command-line mode, parsed exactly before page search ever runs (so a colon-prefixed query can never collide with content): `:h`/`:help` print the keyboard-binding and command table (`.kiln-jump-help`, one non-selectable `pre` grouped into `[ NAVIGATION ]`/`[ SEARCH ]`/`[ VIEW ]`/`[ COMMANDS ]` sections from the script's `HELP_SECTIONS`, descriptions aligned to one site-wide key column) — this is the site's help, its only trigger, the former bottom-right panel and hint being gone; `:version` prints the deployed build stamp (fetched lazily from `version.json`, written by deploy.yml — absent on any local serve, where it reports `local build`); `:intro` prints a centered welcome screen (spaced `KILN` title, the attribution line, the six top-level sections as a `[0x01]`–`[0x06]` grid whose labels are the `1`–`6` keys that open them, and the live page count from the index); any other `:name` answers `E492: Not an editor command: name`, and a bare `:` does nothing, as in vim. A leading `/` is vim's buffer search over the whole site: `/pattern` (two-plus characters) greps section headings and body text case-insensitively — including each page's pre-heading text (intro paragraphs, TL;DR admonitions, metadata blocks), which the search index stores on the page-level entry and joins the section list as an "intro" section landing on the page itself — returning selectable `page › section` rows (the page title alone for intro rows) with a hit excerpt in the path slot that land on the section anchor. The dialect is vim-only — the former GNU `-h`/`--help` flags are gone. All search data comes from the search plugin's `search_index.json` (one page-level entry per page plus one entry per heading section), fetched lazily once per full page load — no extra build step; the site base is derived from the header logo's href. The same component **always mounts inline under the homepage ASCII logo**, input focused, so the homepage is a prompt; its results float as a dropdown over the content below the bar (clamped to the viewport), so matches never change the page height. The homepage card grid in the Markdown is the no-JS fallback: [overrides/main.html](overrides/main.html) stamps `kiln-js` on the html element before first paint and extra.css hides the cards under it (scoped by the ASCII-logo gate — category landing pages keep their card grids); with JS off the class is never set and the cards render as always. The overlay lives on `document.body` (attach-once); the inline variant re-mounts per homepage visit via `KilnUtils.onPageChange`.
- [docs/javascripts/widgets.js](docs/javascripts/widgets.js) — **experimental** interactive content widgets, pure vanilla JS. Progressive enhancement: a page drops an empty `<div class="kiln-widget" data-widget="NAME">` after the block it animates, and this hydrates it by `data-widget` type; with JS off the div is empty and the surrounding prose and code stand alone. Mounting is idempotent, lazy (no-op on widget-less pages), re-run via `KilnUtils.onPageChange`, and marks each node `data-mounted` so a re-run never double-hydrates. One widget so far: `cfb1-keygen` on the CFB1 writeup — type a username, see the serial computed live by that page's verified derivation (so the widget and the prose must stay in step). Widget CSS uses the design tokens and the surface recipe, so it follows the theme; no Material `md-*` internals are hooked, so the build-check canary is unaffected. Loaded last in `extra_javascript`.
...
- [overrides/404.html](overrides/404.html) — the 404 page, a Material template override (wired via `theme.custom_dir: overrides` in mkdocs.yml): blank except a plain `<p class="kiln-404">404</p>`, rendered in JetBrains Mono and centered in the viewport by the `.kiln-404` rules in extra.css (their flex-stretch chain mirrors the homepage's `:has(.kiln-ascii)` rules); an empty `site_nav` block suppresses both sidebars, the header stays as the way back in. GitHub Pages serves the built `site/404.html` for any missing path — asset URLs come out root-relative (`/kiln/...`), so it renders at any depth. Guarded by build-check canaries: `kiln-404` present and `md-sidebar` absent in `site/404.html`.
- [overrides/main.html](overrides/main.html) — extends Material's base template only to inject one inline script into `extrahead`: it stamps `kiln-js` on the html element before first paint, so the homepage's no-JS fallback cards never flash before quick-jump.js mounts the jump bar (with JS off the class is never set and the cards render). 404.html extends this template, harmlessly. Guarded by a build-check canary (`kiln-js` present in `site/index.html`).
...
- Fixed chrome decisions, not to be reverted casually: content column capped at 900px; sidebars hidden by default site-wide (revealed per-session with `s`) and disabled entirely on the homepage, drawer toggle included; content tables full-width with `table-layout: fixed` and a label-style header row; Material footer hidden; "Copied to clipboard" dialog suppressed (checkmark feedback instead); scroll-to-top/bottom buttons replaced by keyboard navigation (key-nav.js); the hex progress chip bottom-right fades with scroll activity (`.scroll-progress--visible`, toggled by page-chrome.js); the homepage is the jump bar (quick-jump.js), with the Markdown card grid kept solely as the no-JS fallback — both paths must keep working.

...
- [docs/stylesheets/extra.css](docs/stylesheets/extra.css) — all custom styling: JetBrains Mono as the global font, color variables in `:root` with `[data-md-color-scheme="slate"]` dark-mode overrides, ASCII logo sizing variables, card grid, register cards, sidebar styling (sticky nav titles + injected gradient fade), the hex progress chip, and the key-help panel. The Material footer is hidden entirely (`.md-footer { display: none }`) — its prev/next links still feed key-nav.js. Notes:
    ...
    - Material's header search bar and palette toggle are hidden (`.md-search`, `[data-md-component="palette"]`) — `S` summons the jump palette and `M` cycles the color scheme instead; the search plugin still runs for the palette's index.
...
- [docs/javascripts/key-nav.js](docs/javascripts/key-nav.js) — keyboard navigation, replacing the former scroll-to-top/bottom buttons: `T`/`D` scroll to top/bottom, `J`/`K` step down/up, `[`/`]` jump to the previous/next `h2` section (anchor line just below the sticky header), `<`/`>` follow the previous/next page in nav order, `0` goes home (clicks the header `md-logo` link), `1`–`6` open the top-level categories in sidebar nav order (clicks the nth top-level `md-nav__item` link), `M` cycles the color palette (advances Material's `__palette` radio group — dark and light only; the header toggle button is hidden), `H` reveals/re-hides both sidebars, which are **hidden by default site-wide** and **disabled entirely on the homepage** (`kiln-sidebars-shown` on `body` — desktop-only in extra.css because the sidebars are off-canvas drawers below Material's 76.25em breakpoint; the class survives `navigation.instant`, so the choice holds across pages until reload, and never applies on the homepage), `?` toggles the `.key-help` panel (Escape closes it). Page switching clicks Material's footer prev/next links (`md-footer__link--prev/next`), which exist in the DOM only because `navigation.footer` is enabled in mkdocs.yml — the footer itself stays `display: none`; the click path keeps `navigation.instant` in charge. Keys are inert while typing and in Ctrl/Alt/Meta chords; Material's own `P`/`N` (prev/next) bindings stay, while its search keys are neutralized — `S` is taken over by quick-jump.js for the palette and `F`/`/` are swallowed by a capture-phase listener, so nothing summons the hidden search form (all still type in inputs); scrolling is smooth unless `prefers-reduced-motion`. Attach-once (no per-page init); the help panel lives on `document.body`, surviving instant navigation.
- [docs/javascripts/quick-jump.js](docs/javascripts/quick-jump.js) — jump-to-page palette, the site's navigation and its search (Material's search UI is hidden; the search plugin only feeds this). `S`, `` ` `` or Ctrl/Cmd+K summon the prompt — the homepage's inline bar when mounted, an overlay elsewhere (`S` preempts Material's search handler in capture phase): fuzzy-match every page by title and path (substring hits outrank subsequence scatter; whitespace-separated terms must all match, so `csapp 3` works), ArrowDown/ArrowUp select, Enter clicks the result's real link (keeping `navigation.instant` in charge), Escape or a backdrop click closes; the prompt rests as a bar showing a `█` terminal cursor (steady while unfocused, blinking while focused, the Linux console convention) plus a faint `jump to a page` placeholder (an empty query still shows no results; the block cursor hides and the native caret takes over at the first typed character, driven by `:placeholder-shown`), and the current page is never offered. The page list comes from the search plugin's `search_index.json` (entries without a `#`), fetched lazily once per full page load — no extra build step; the site base is derived from the header logo's href. The same component **always mounts inline under the homepage ASCII logo**, input focused, so the homepage is a prompt; its results float as a dropdown over the content below the bar (clamped to the viewport), so matches never change the page height. The homepage card grid in the Markdown is the no-JS fallback: [overrides/main.html](overrides/main.html) stamps `kiln-js` on the html element before first paint and extra.css hides the cards under it (scoped by the ASCII-logo gate — category landing pages keep their card grids); with JS off the class is never set and the cards render as always. The overlay lives on `document.body` (attach-once); the inline variant re-mounts per homepage visit via `KilnUtils.onPageChange`.
...
- [docs/javascripts/ascii-logo.js](docs/javascripts/ascii-logo.js) — generative homepage ASCII logo. Renders `CONFIG.text` ("Kiln") in UnifrakturMaguntia on an offscreen canvas, samples glyph coverage per character cell, and maps it onto a 70-character density ramp inside `pre.kiln-ascii`, sized to fit the content width and a viewport-height budget (no scrollbars). Static until the cursor moves; pointer movement drives a shimmer whose intensity envelope eases in/out. Key facts:
    ...
    - Waits for the two font faces the build actually uses — the logo font for the canvas and the pre's rendered JetBrains Mono, whose cell sizes the grid — **not** `document.fonts.ready`, which would stall the logo behind every face the page pulls on a cold load; it rebuilds on late font loads (`loadingdone`), on window resize, and on container resize (a `ResizeObserver` on the pre's parent — the H sidebar toggle changes the content width with no window resize, and a stale grid overflows the narrowed column).
    - With JS on (`kiln-js`), extra.css turns the pre-generation pre into an invisible placeholder already at the art's final size (container width × `--kiln-logo-ink-aspect`, capped at the 42vh budget mirrored from `CONFIG.maxViewportHeightRatio`), so a cold load shows no title flash and no layout jump when the art lands (it fades in). With JS off the pre renders its plain-text fallback styled as a title. The `kiln-ascii--generated` class switches it to grid sizing.
    - To change the word: `CONFIG.text` **plus** the fallback text/`aria-label` in [docs/index.md](docs/index.md) **plus** re-deriving `--kiln-logo-ink-aspect` in extra.css (ink bounding-box height/width of the new text via `measureText`). To change the font: `CONFIG.fontFamily` and `CONFIG.fontStylesheetUrl`, and re-derive the aspect likewise.

...
- [mkdocs.yml](mkdocs.yml) — site config: theme + three-way palette (light/dark/system), `theme.custom_dir: overrides` (the 404 template and the pre-paint home-view script), features (`navigation.instant`, `navigation.prune`, `navigation.footer` for key-nav.js, etc.), markdown extensions, `extra_javascript` load order, and `validation` settings. Link problems are escalated to warnings so `--strict` fails on them; the homepage is intentionally not in `nav`, so `nav.omitted_files` stays at `info`. `theme.font: false` disables Material's Google-Fonts loading — JetBrains Mono comes from the `@import` in extra.css instead. The `nav:` section must be updated manually when adding pages or sections.
...
- [docs/javascripts/key-nav.js](docs/javascripts/key-nav.js) — keyboard navigation, replacing the former scroll-to-top/bottom buttons: `T`/`D` scroll to top/bottom, `J`/`K` step down/up, `[`/`]` jump to the previous/next `h2` section (anchor line just below the sticky header), `<`/`>` follow the previous/next page in nav order, `0` goes home (clicks the header `md-logo` link), `1`–`4` open the top-level sections in sidebar nav order (clicks the nth top-level `md-nav__item` link), `M` cycles the color palette (advances Material's `__palette` radio group, same as its toggle button), `H` reveals/re-hides both sidebars, which are **hidden by default site-wide** (`kiln-sidebars-shown` on `body` — desktop-only in extra.css because the sidebars are off-canvas drawers below Material's 76.25em breakpoint; the class survives `navigation.instant`, so the choice holds across pages until reload), `?` toggles the `.key-help` panel (Escape closes it). The panel also carries the home-view switch button (`.key-help-switch`), which fires the `kiln:home-view-toggle` event handled by quick-jump.js — the mode logic lives there, and the button's two labels swap via the html mode class in extra.css, so key-nav holds no state for it. Page switching clicks Material's footer prev/next links (`md-footer__link--prev/next`), which exist in the DOM only because `navigation.footer` is enabled in mkdocs.yml — the footer itself stays `display: none`; the click path keeps `navigation.instant` in charge. Keys are inert while typing and in Ctrl/Alt/Meta chords; Material's own `S` (search) and `P`/`N` (prev/next) bindings stay, while its extra `/` search key is swallowed by a capture-phase listener so `S` alone opens search (it still types in inputs) — its other extra search key `F` is claimed by link-hints.js the same way; scrolling is smooth unless `prefers-reduced-motion`. Attach-once (no per-page init); the help panel lives on `document.body`, surviving instant navigation.
- [docs/javascripts/quick-jump.js](docs/javascripts/quick-jump.js) — jump-to-page palette and the home-view mode. `` ` `` or Ctrl/Cmd+K toggles an overlay palette on any page: fuzzy-match every page by title and path (substring hits outrank subsequence scatter; whitespace-separated terms must all match, so `csapp 3` works), ArrowDown/ArrowUp select, Enter clicks the result's real link (keeping `navigation.instant` in charge), Escape or a backdrop click closes; the prompt rests as a clean bar (an empty query shows nothing), and the current page is never offered. The page list comes from the search plugin's `search_index.json` (entries without a `#`), fetched lazily once per full page load — no extra build step; the site base is derived from the header logo's href. The same component mounts inline under the homepage ASCII logo in **jump-bar mode**: an html class (`kiln-home-jump`, persisted as `kiln-home-view` in localStorage) hides the homepage card grid via extra.css (scoped by the ASCII-logo gate — section landing pages keep their card grids in every mode) and the inline palette, input focused, becomes the homepage; its results float as a dropdown over the content below the bar, so matches never change the page height. The switch button in key-nav's help panel toggles it (via `kiln:home-view-toggle`); [overrides/main.html](overrides/main.html) applies the stored class before first paint so the homepage never flashes the other view; with JS off nothing sets the class and the cards render as always. The overlay lives on `document.body` (attach-once); the inline variant re-mounts per homepage visit via `KilnUtils.onPageChange`.
- [docs/javascripts/link-hints.js](docs/javascripts/link-hints.js) — follow any visible link from the keyboard. `F` overlays a short fixed-length label (home-row-first alphabet, prefix-free set) on every link in the viewport — cards, table rows, cross-references, sidebar and header alike; typing a label clicks its link, so internal links stay with `navigation.instant` and external ones keep external-links.js's new-tab behavior. The typed prefix highlights in accent; Backspace untypes; Escape, any scroll/resize, a mouse press, or a stray key cancels (chip positions are viewport-pinned, so anything that invalidates them clears the layer). While hints are active all keys are consumed by a capture-phase listener, so half-typed labels never leak scrolls or page switches. Attach-once; the label layer is rebuilt per activation.
...
- [overrides/main.html](overrides/main.html) — extends Material's base template only to inject one inline script into `extrahead`: it applies the stored home-view mode class (`kiln-home-jump`, see quick-jump.js) to the html element before first paint, so a jump-bar-mode visitor never sees the card grid flash. It exists solely for the pre-paint timing — quick-jump.js re-applies the class as a late fallback. 404.html extends this template, harmlessly. Guarded by a build-check canary (`kiln-home-jump` present in `site/index.html`).

...
- [docs/javascripts/page-chrome.js](docs/javascripts/page-chrome.js) — sidebar title fade, scroll-to-top/bottom buttons, and the hex reading-progress readout (`.scroll-progress`, first chip in the scroll-button cluster: scroll offset over full scroll range as lowercase hex byte offsets, `0x1388/0x8d88`, offset padded to the range's width so the chip never resizes; empty on unscrollable pages). Unlike the buttons, the chip shows only around scroll activity — `.scroll-progress--visible` is toggled on scroll and dropped ~1s after scrolling stops, fading via opacity so its flex slot (and the buttons) never shift. Init functions are idempotent and re-run via `KilnUtils.onPageChange` because `navigation.instant` removes injected DOM on navigation.
...
- **Surface recipe.** Every raised element — home cards, register cards, code `pre` blocks, the header, scroll buttons — is the same construction: `--color-surface` background, 1px `--color-border` border, 4px border-radius (3px for inline code), and **flat at rest** (no box-shadow until hover). Backgrounds elsewhere are `--color-bg` (body, sidebars, search form).

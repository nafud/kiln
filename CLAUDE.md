...
- [docs/javascripts/ascii-logo.js](docs/javascripts/ascii-logo.js) — generative homepage ASCII logo. Renders `CONFIG.text` ("Kiln") in UnifrakturMaguntia on an offscreen canvas, samples glyph coverage per character cell, and maps it onto a 70-character density ramp inside `pre.kiln-ascii`, sized to fit the content width and a viewport-height budget (no scrollbars). Static until the cursor moves; pointer movement drives a shimmer whose intensity envelope eases in/out. Key facts:
    ...
    - Before generation the pre holds its plain-text fallback; with JS on (`kiln-js`), extra.css renders it as an invisible placeholder box already at the art's final size (container width × `--kiln-logo-ink-aspect`, capped at the 42vh budget mirrored from `CONFIG.maxViewportHeightRatio`), so a cold load shows no title flash and no layout jump when the art lands (it fades in). With JS off the fallback renders styled as a title. The `kiln-ascii--generated` class switches the pre to grid sizing.
    - To change the word: `CONFIG.text` **plus** the fallback text/`aria-label` in [docs/index.md](docs/index.md) **plus** re-deriving `--kiln-logo-ink-aspect` in extra.css (ink bounding-box height/width via `measureText`). To change the font: `CONFIG.fontFamily` and `CONFIG.fontStylesheetUrl`, and re-derive the aspect likewise.

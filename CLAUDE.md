...
- [docs/javascripts/quick-jump.js](docs/javascripts/quick-jump.js) — jump-to-page palette, the site's navigation and its search (Material's search UI is hidden; the search plugin only feeds this). `` ` `` or Ctrl/Cmd+K summons the prompt — the homepage's inline bar when mounted, an overlay elsewhere: fuzzy-match every page by title and path (substring hits outrank subsequence scatter; whitespace-separated terms must all match, so `csapp 3` works), ArrowDown/ArrowUp select, Enter clicks the result's real link (keeping `navigation.instant` in charge), Escape or a backdrop click closes; the prompt rests as a bar showing a `█` terminal cursor (steady while unfocused, blinking while focused, the Linux console convention) plus a faint `type :h for help` placeholder (an empty query still shows no results; the block cursor hides and the native caret takes over at the first typed character, driven by `:placeholder-shown`), and the current page is never offered. A leading `:` is vim command-line mode, parsed exactly before page search ever runs (so a colon-prefixed query can never collide with content): `:h`/`:help` print the keyboard-binding and command table (`.kiln-jump-help`, one non-selectable `pre` grouped into `[ NAVIGATION ]`/`[ SEARCH ]`/`[ VIEW ]`/`[ COMMANDS ]` sections from the script's `HELP_SECTIONS`, descriptions aligned to one site-wide key column) — this is the site's help, its only trigger, the former bottom-right panel and hint being gone; `:version` prints the deployed build stamp (fetched lazily from `version.json`, written by deploy.yml — absent on any local serve, where it reports `local build`); `:intro` prints a centered welcome screen (spaced `KILN` title, the attribution line, the six top-level sections as a `[0x01]`–`[0x06]` grid whose labels are the `1`–`6` keys that open them, and the live page count from the index); any other `:name` answers `E492: Not an editor command: name`, and a bare `:` does nothing, as in vim. A leading `/` is vim's buffer search over the whole site: `/pattern` (two-plus characters) greps section body text — the `#fragment` index entries the page view drops — case-insensitively, returning selectable `page › section` rows with a hit excerpt in the path slot that land on the section anchor. The dialect is vim-only — the former GNU `-h`/`--help` flags are gone. The page list comes from the search plugin's `search_index.json` (entries without a `#`), fetched lazily once per full page load — no extra build step; the site base is derived from the header logo's href. The same component **always mounts inline under the homepage ASCII logo**, input focused, so the homepage is a prompt; its results float as a dropdown over the content below the bar (clamped to the viewport), so matches never change the page height. The homepage card grid in the Markdown is the no-JS fallback: [overrides/main.html](overrides/main.html) stamps `kiln-js` on the html element before first paint and extra.css hides the cards under it (scoped by the ASCII-logo gate — category landing pages keep their card grids); with JS off the class is never set and the cards render as always. The overlay lives on `document.body` (attach-once); the inline variant re-mounts per homepage visit via `KilnUtils.onPageChange`.
- [docs/javascripts/widgets.js](docs/javascripts/widgets.js) — **experimental** interactive content widgets, pure vanilla JS. Progressive enhancement: a page drops an empty `<div class="kiln-widget" data-widget="NAME">` **after** the static block it animates, and this hydrates it by `data-widget` type; with JS off the div is empty and the static example above stands alone (the reproducible example is never lost). Mounting is idempotent, lazy (no-op on widget-less pages), re-run via `KilnUtils.onPageChange`, and marks each node `data-mounted` so a re-run never double-hydrates. Two widgets, both on the CFB1 writeup and both reimplementing that page's verified derivation (so the widget and the prose must stay in step): `cfb1-keygen` (type a username, see the serial live) and `cfb1-stepper` (walk the byte-wide `lea`/`xor al`/`add al`/`movzx` sequence, watching AL and the ZF/SF/CF/OF/PF flags — reuses the `.register-grid`/`.register-card` classes). Widget CSS uses the design tokens and the surface recipe, so it follows the theme; no Material `md-*` internals are hooked, so the build-check canary is unaffected. Loaded last in `extra_javascript`.
